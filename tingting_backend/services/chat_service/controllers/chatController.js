  ...
  sendMessage: async (req, res) => {
    try {
      ...
      if (messageType === "text" && !content?.trim() && !replyMessageId) {
        ...
        return res
          .status(400)
          .json({ message: "Text message cannot be empty" });
      }
      ...
      if (["image", "file", "video"].includes(messageType) && !linkURL) {
        ...
        return res
          .status(400)
          .json({ message: "File message must have a linkURL" });
      }
  ...
  getMessages: async (req, res) => {
    try {
      ...
      const messages = await Message.find({ conversationId })
        .sort({ createdAt: 1 })
        .populate("userId", "username avatar"); // Populate thông tin người gửi
    ...
    try {
      ...
      if (
        ...
        return res
          .status(400)
          .json({
            message: "Vui lòng cung cấp một hoặc nhiều ID tin nhắn để xóa.",
          });
      }
      ...
      if (messagesToDelete.length === 0) {
        return res
          .status(404)
          .json({ message: "Không tìm thấy tin nhắn nào để xóa." });
      }
      ...
      if (deletedMessages.deletedCount > 0) {
        ...
        console.log(`[BACKEND] Emitting messageRevoked to ${conversationId}:`, {
          messageIds,
        });

        // Sau khi xóa, cập nhật thông tin cuộc trò chuyện (lastMessage)
        const lastMessageAfterDelete = await Message.findOne({
          conversationId,
        }).sort({ createdAt: -1 });
        ...
        await Conversation.findByIdAndUpdate(
          conversationId,
          {
            ...
            lastMessage: lastMessageAfterDelete
              ? lastMessageAfterDelete._id
              : null,
            updateAt: Date.now(),
          },
          { new: true }
        ...
        if (updatedConversation) {
          ...
          console.log(
            `[BACKEND] Emitting conversationUpdated to ${conversationId}:`,
            updatedConversation
          );
        }
      }
      ...
      res
        .status(200)
        .json({
          message: `Đã xóa ${deletedMessages.deletedCount} tin nhắn và các tài nguyên liên quan.`,
        });
    } catch (error) {
  ...
  forwardMessage: async (req, res) => {
    try {
      ...
      if (
        ...
        return res
          .status(400)
          .json({ message: "Missing required information for forwarding." });
      }
      ...
      for (const targetConversationId of targetConversationIds) {
        ...
        try {
          ...
          try {
            ...
            if (!updateResult) {
              console.log(
                `Không tìm thấy Conversation để cập nhật: ${targetConversationId}`
              );
            }
          } catch (updateError) {
          ...
          if (io) {
            ...
            console.log(
              `Forwarded message emitted to conversation ${targetConversationId}:`,
              savedMessage
            );
          } else {
